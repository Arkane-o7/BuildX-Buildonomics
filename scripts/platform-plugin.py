...
For UPI, select the merchant's UPI option and advance to its real QR/intent/payment request when authorized. Tell the user the exact merchant and amount and ask them to approve in their own UPI app. Never ask for or enter a UPI PIN. Do not invent a collect request or payee. Pause at bank authentication, then resume after the user says they approved and inspect the merchant status. For cards, use only a user-selected card already stored securely at the merchant or an actually integrated provider vault. Never store PAN/CVV/OTP in the agent, config or chat. Let the user complete any bank challenge directly. There is currently no AgentPass card-vault or delegated-UPI adapter. Do not bypass bank challenges or promise unattended payment.
