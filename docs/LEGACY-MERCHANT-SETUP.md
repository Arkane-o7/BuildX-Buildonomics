...
In Razorpay **Test Mode**, open **Account & Settings → Webhooks**. The active event webhook is `https://agentpass-buildx.vercel.app/api/webhooks/razorpay?integration=agentpass-test`, subscribed to `payment.captured` with the same `RAZORPAY_WEBHOOK_SECRET` as the deployed server. The query parameter identifies the replacement configuration and uses the same application handler. If the local private file `.agentpass-private/RAZORPAY-WEBHOOK.txt` exists, it contains the event's webhook settings. Keep its secret private. Configure automatic capture: the app only completes orders after Razorpay reports `captured`.
...
UPI-only checkout currently reports **No appropriate payment method found** for this merchant in desktop and emulated mobile-web testing. The signed-in Razorpay Test Mode **Payment Configuration → Standard payment options** and its own checkout preview show Cards, Netbanking, Wallets and PayLater only. UPI provider completion is therefore **not yet verified**. Razorpay documents its [Payment Methods settings](https://razorpay.com/docs/payments/dashboard/account-settings/payment-methods/) as Live Mode only; there is no corresponding Test Mode activation switch to look for. Ask provider support about sandbox UPI availability; do not present the rehearsal UPI flow as a captured provider payment.

**Webhook verified (17 September 2026):** the original webhook had no secret, and Razorpay rejected attempts to edit it. A replacement webhook with the matching server secret was created; the old unsigned webhook is disabled. A fresh ₹20 test-card payment completed through an isolated checkout whose browser callback did not call AgentPass verification. The provider webhook alone completed the purchase and produced a valid signed receipt. The verified request is `webhook-only-verification-001`. Do not create another duplicate webhook during normal setup.
