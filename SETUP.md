...
UPI-only checkout currently reports **No appropriate payment method found** for this merchant in desktop and emulated mobile-web testing. UPI provider completion is therefore **not yet verified**. Inspect UPI under Razorpay Test Mode's payment methods or contact provider support; do not present the rehearsal UPI flow as a captured provider payment. The webhook was saved by the owner; confirm `payment.captured` delivery HTTP 200 separately in the provider dashboard.
